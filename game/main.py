...
from game_map import is_inside
...
def print_map(game_map, water, water_capacity, lives, score):
    ...
    print(border_tile * (width + 2))
    print("\n💡 Подсказки:")
    print("❤️ +1 жизнь = 3 ⭐   |   🪣 +1 ёмкость воды = 5 ⭐   |   💾 Нажми F — сохранить игру")
...
def main():
    ...
    if load:
        ...
        fire_timers = {tuple(map(int, k.split(','))): v for k, v in data['fire_timers'].items()}
        weather_map = {tuple(map(int, k.split(','))): v for k, v in data['weather_map'].items()}
    ...
    while True:
        ...
        if tick % 5 == 0:
            active_fires = sum(
                1 for y in range(height) for x in range(width) if game_map[y][x] == TILES['fire']
            )
            ...
            if fires_to_add > 0:
                ...
                for y in range(height):
                    for x in range(width):
                        if game_map[y][x] == TILES['fire'] and (y, x) not in fire_timers:
        ...
        for (y, x), time_left in list(fire_timers.items()):
            ...
            if fire_timers[(y, x)] <= 0:
                ...
                del fire_timers[(y, x)]
                if score > 0:
                    score -= 1  # штраф за сгоревшее дерево
        ...
        tree_count = sum(
            1 for row in game_map for cell in row if cell == TILES['tree']
        )
        if tree_count == 0:
        ...
        if lives <= 0:
            print("💀 Вертолет разбился! GAME OVER.")
            break
